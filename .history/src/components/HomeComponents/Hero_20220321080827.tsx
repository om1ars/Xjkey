...
const Hero = ({ title, subtitle, line }: any) => {
  const [showBanner, setShowBanner] = useState(true);
  return (
    <>
      <div className="relative h-[350px] w-full bg-mainBg text-white sm:h-[45vh] md:h-[400px]">
        {/* <Image
        ...
        <div className="bg-neutral-900/70 absolute inset-0">
          <div className="mx-auto max-w-screen-lg">
            <div className="m-6 min-h-[60px]"></div>
            <div className="mx-4   flex flex-col items-center text-center">
              <h1
                className={`text-xl font-extrabold uppercase tracking-wide text-white sm:text-4xl md:text-3xl lg:text-4xl ${
                  line && "line-through	"
                }`}
              >
                {title}
...
const SearchForm = () => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="pointer-events-none block px-2 text-textBold group-focus-within:text-textBold">
          <svg
            className="h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
        ...
        <input
          className="w-full appearance-none border-none border-transparent p-2 text-xl leading-6 text-textBold placeholder-textBold outline-none focus:border-transparent focus:ring-0"
          type="text"
        ...
        <button
          type="submit"
          className="hidden h-11 shrink-0  bg-buttonBg px-2 font-semibold text-white transition-colors duration-500 ease-in-out hover:bg-buttonHover sm:block"
        >
          Discover Now
        </button>
      </div>
